 ...
 *
 * CompanyDashboardController will control the company dashboard actions
 */
  .controller('CompanyDashboardController',
  [
    '$rootScope', '$scope', '$state', '$stateParams', '$modal', '$log', '$filter', 'LoopBackAuth', 'UserDetail', 'CompanyRequest', 'EndorsetSent', 'Question', 'EndorsementService', 'Endorset', 'notificationService', 'ngToast', 'PubSub', 'LoginService', 'AssignEndorsets', '$http',
    function($rootScope, $scope, $state, $stateParams, $modal, $log, $filter, LoopBackAuth, UserDetail, CompanyRequest, EndorsetSent, Question, EndorsementService, Endorset, notificationService, ngToast, PubSub, LoginService, AssignEndorsets, $http) {
      ...
      CompanyRequest.updateProfessionalUserId({msg: 'Hello World'}, function(res){
        //console.log(res);
      });
      ...
      $scope.mainTabs = {
        ...
        myendorsets: {name: "My open Endorsets", active: false, disabled: false}
      };
      ...
      $scope.openEndorserEndorsement = function(endorsement){
        ...
        var modalInstance = $modal.open({
          ...
          controller: 'ModalEndorsementCtrl',
          size: 'lg',
          resolve: {
            items: function () {
              return {
      ...
      $scope.openMyEndorsetTab = function(currentInstance, notification){
        ...
        $scope.mainTabs.requests.active = false;
        $scope.mainTabs.endorsets.active = false;
        $scope.mainTabs.myendorsets.active = true;
                ...
                /*and: [
                  {
                    ...
                    status: 1,
                    "or": [
                      {"firstName": {like: '%'+ viewValue +"%"}}, {"lastName": {like: '%'+ viewValue +"%"}}
                    ]
                  }
                ]*/
              },
...
//dashboard endorsement modal instance controller
  .controller('ModalEndorsementCtrl', function ($rootScope, $scope, $state, $modalInstance, items, $filter, LoopBackAuth, UserDetail, EndorsementService, Endorset, HrFeedback) {
    $scope.user = LoopBackAuth.currentUserData;
